package edu.buaa.rsutils.jerasure;

import edu.buaa.rsutils.cauchy.Cauchy;
import edu.buaa.rsutils.matrix.*;
import edu.buaa.rsutils.utils.CodingUtils;
import edu.buaa.rsutils.utils.FileUtils;

import java.io.*;
import java.util.SortedMap;
import java.util.TreeMap;

import static edu.buaa.rsutils.utils.CalcUtils.*;
...
public class Decoder {
	...
	public Decoder(String file, int k, int m, int w) {
		this.original = new File(FileUtils.generateDecoderPath(file));    // 恢复成的文件路径
		this.k = k;
		this.m = m;
		this.w = w;

		this.k_parts = FileUtils.collectFiles(file, "k", k);
		this.m_parts = FileUtils.collectFiles(file, "m", m);
